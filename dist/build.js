...
import { loadTheme, renderPage, } from "./theme/index.js";
import { scanAssets, resolveAssetRef, assetType, } from "./assets.js";
...
export async function build(options = {}) {
    // 考虑创建一个 utils.logger ？
    const buildLog = (msg) => console.log("  [build]: " + msg);
    ...
    const files = seqRead(contentRoot, cwd);
    ...
    const virtualPages = await seqGenerate(site, generators);
    ...
    const allPages = [...filteredPages, ...virtualPages];
    ...
    if (scanned.stray.length > 0) {
        console.warn(`[warn] 以下散落文件未归属任何文章或全局资源，已忽略：\n  ${scanned.stray.join("\n  ")}`);
    }
    ...
    const renderCtx = { config: siteConfig, assets: assetRegistry };
    const results = [];
    for (const page of allPages) {
        ...
        if (!renderer)
            throw new Error("未注册任何 renderer");
    ...
    // ---- write ----
    await hooks.beforeWrite.call(results.map((r) => r.page), assets);
...
// ---------- 内部工具 ----------
function checkUrlConflicts(pages) {
    const seen = new Map();
...
function resolveCover(page, ctx) {
    if (!page.cover)
        return;
    const resolveOne = (ref) => {
        ...
        if (resolved === null) {
            throw new Error(`[${page.id}] cover 引用未命中任何资源: "${ref}"（已在文章专属目录与全局 assetsDir 查找）`);
        }
        return resolved;
    };
...
async function compileLess(source, filename) {
