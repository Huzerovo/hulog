...
export async function initCorePlugins(
  site: Site,
  cwd: string,
): Promise<{ registries: Registries; scoped: ScopedAPIs }> {
...
export function registerCoreGenerators(api: GeneratorAPI): void {
  homeGenerator(api);
...
export async function loadSitePlugins(
  scoped: ScopedAPIs,
  cwd: string,
  site: Site,
): Promise<void> {
