...
/** 字符串按 "/" 拆段并去除空白段 */
function splitSegs(name: string): string[] {
  return name
    .split(SEP)
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
...
function walk(value: unknown, parent: CategoryPath, out: CategoryPath[]): void {
  // 标量（字符串/数字/布尔）→ 叶子，支持 "父/子" 路径写法
  if (typeof value !== "object" || value === null) {
    if (value == null) return;
    const segs = splitSegs(String(value));
    if (segs.length > 0) out.push([...parent, ...segs]);
    return;
  }
  if (Array.isArray(value)) {
    for (const item of value) walk(item, parent, out);
    return;
  }
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    const p = [...parent, ...segs];
    if (isEmptyValue(child)) out.push(p);
    else walk(child, p, out);
  }
}
 ...
 */
export function parseCategories(value: unknown): CategoryPath[] {
...
/** 路径 → 展示字符串："父/子" */
export function categoryPathToString(path: CategoryPath): string {
 ...
 */
export function categoryPathToUrl(
  path: CategoryPath,
  base = "/categories",
): string {
 ...
 */
export function buildCategoryTree(paths: CategoryPath[]): CategoryNode[] {
  interface MutableNode extends CategoryNode {
  ...
  const finalize = (map: Map<string, MutableNode>): CategoryNode[] => {
