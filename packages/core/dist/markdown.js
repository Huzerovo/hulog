...
async function getHighlighter() {
    if (highlighter)
        return highlighter;
    const langs = await Promise.all(COMMON_LANGS.map((l) => l.then((m) => m.default ?? m)));
...
/** 资源引用解析 */
function rehypeResolveAssets(page, resolve) {
    return () => (tree) => {
        visit(tree, "element", (node) => {
            if (node.tagName === "img") {
...
export async function renderMarkdown(rawContent, page, ctx) {
    ...
    processor
        ...
        .use(rehypeResolveAssets(page, ctx.resolve));
